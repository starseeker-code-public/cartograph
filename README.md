...
> **Project status**: V1 in progress. Schema and bootstrap complete (Phase 1–2). See the [phase plan](#phase-plan) below.
...
### Phase 3 — Service-area management 🔜 next
...
### Phase 7 — Single-driver TSP optimizer
...
### Phase 8 — Coverage analytics (H3)

Pickup + delivery H3 hex IDs (resolution 8, ≈ 0.7 km hex) are stored on every order. Aggregation query groups by hex and counts orders over a time range. Frontend renders the H3 boundaries via `@turf/h3-grid` with a sequential color scale. Resolution selectable 7–9.
...
│   │   │   └── tests/
│   │   └── .env.example
│   └── web/                ← React + MapLibre (Phase 9, scaffold pending)
├── infra/
│   ├── compose/
...
- HTTPS via Caddy auto-TLS.
- Tailscale-only API access by default — public exposure is opt-in.
